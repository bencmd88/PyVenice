...
class ModelCapabilities(BaseModel):
    ...
    quantization: Literal["fp8", "fp16", "not-available"] = Field(description="The quantization type")
    supportsFunctionCalling: bool = Field(description="Does the model support function calling?")
    supportsReasoning: bool = Field(description="Does the model support reasoning with <thinking> blocks?")
    supportsResponseSchema: bool = Field(description="Does the model support response schema?")
    ...
    supportsLogProbs: bool = Field(description="Does the model support logprobs parameter?")


class ModelSpec(BaseModel):
    """Model specification details."""
    availableContextTokens: Optional[int] = Field(None, description="Context length for text models")
    ...
    defaultParameters: Optional[Dict[str, Any]] = Field(None, description="Default parameters")
    description: Optional[str] = Field(None, description="Model description")
    developer: Optional[str] = Field(None, description="Model developer")
    disabledInPlayground: Optional[bool] = Field(None, description="Disabled in playground?")
    earliestAccess: Optional[str] = Field(None, description="Earliest access tier")
    hasDisclaimerPopup: Optional[bool] = Field(None, description="Has disclaimer popup?")
    humanName: Optional[str] = Field(None, description="Human-readable name")
    imageGenerations: Optional[int] = Field(None, description="Number of image generations")
    isAvailable: Optional[bool] = Field(None, description="Is model available?")
    maxCompletionTokens: Optional[int] = Field(None, description="Maximum completion tokens")
    ...
    upscalingFactor: Optional[int] = Field(None, description="Upscaling factor for image models")
...
class Models(BaseResource):
    ...
    def list(
        self,
        type: Optional[Literal["embedding", "image", "text", "tts", "upscale", "all", "code"]] = None,
        ...
        if not type and not force_refresh and self._models_cache:
            if self._models_cache_time and datetime.now() - self._models_cache_time < self.CACHE_DURATION:
    ...
    def get_capabilities(self, model_id: str, force_refresh: bool = False) -> Optional[ModelCapabilities]:
        """
    ...
    def supports_parameter(self, model_id: str, parameter: str, force_refresh: bool = False) -> bool:
        """
