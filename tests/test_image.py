...
class TestImageGeneration:
    ...
    @respx.mock
    def test_generate_image(self, respx_mock, mock_image_response, mock_compatibility_mapping, client):
    ...
    @respx.mock
    def test_generate_image_with_params(self, respx_mock, mock_image_response, mock_compatibility_mapping, client):
    ...
    def test_upscale_image_base64(self, respx_mock, client):
        ...
        respx_mock.post("https://api.venice.ai/api/v1/image/upscale").mock(
            return_value=httpx.Response(200, content=fake_image_data, headers={"content-type": "image/png"})
        )
        
        image_gen = ImageGeneration(client)
        input_image = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAGA0"
        
        result = image_gen.upscale(
            image=input_image,
            scale=2,
            enhance=True
        )
    ...
    def test_upscale_image_from_file(self, mock_file, respx_mock, client):
        ...
        result = image_gen.upscale(
            image=Path("/fake/path/image.jpg"),
            scale=2
        )
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_generate_async(self, respx_mock, mock_image_response, mock_compatibility_mapping, client):
    ...
    async def test_upscale_async(self, respx_mock, client):
        ...
        result = await image_gen.upscale_async(
            image=input_image,
            scale=2
        )
