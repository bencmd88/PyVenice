...
def setup_audio_mocks(respx_mock):
    ...
    respx_mock.get("https://api.venice.ai/api/v1/models/compatibility_mapping").mock(
        return_value=httpx.Response(200, json={"data": {}, "object": "list", "type": "text"})
    )
...
class TestAudio:
    ...
    def test_create_speech_basic(self, respx_mock, client):
        ...
        respx_mock.post("https://api.venice.ai/api/v1/audio/speech").mock(
            return_value=httpx.Response(200, content=fake_audio_data, headers={"content-type": "audio/mpeg"})
        )
    ...
    def test_create_speech_with_response_format(self, respx_mock, client):
        ...
        fake_audio_data = b"fake-audio-data"
        respx_mock.post("https://api.venice.ai/api/v1/audio/speech").mock(
            return_value=httpx.Response(200, content=fake_audio_data, headers={"content-type": "audio/wav"})
        )
        
        audio = Audio(client)
        response = audio.create_speech(
            input="Test with WAV format",
            voice="am_echo",
            response_format="wav"
        )
    ...
    async def test_create_speech_async(self, respx_mock, client):
        ...
        audio = Audio(client)
        response = await audio.create_speech_async(
            input="Async test",
            voice="shimmer"
        )
...
class TestAudioIntegration:
    ...
    def test_real_speech_with_different_voices(self, skip_if_no_api_key, integration_api_key):
        ...
        for voice in voices_to_test:
            response = audio.create_speech(
                input=f"This is {voice} speaking.",
                voice=voice
            )
    ...
    def test_real_speech_with_different_formats(self, skip_if_no_api_key, integration_api_key):
